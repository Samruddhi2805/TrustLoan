...
export default function Navbar({ account, connectWallet, disconnectWallet, activeUserCount }) {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 p-3 sm:p-4 px-3 sm:px-6 flex justify-between items-center bg-navy-bg/50 backdrop-blur-md border-b border-glass-border">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 glass-card px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-glass-border">
      ...
      <div>
        {account ? (
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="flex items-center gap-2 sm:gap-3 glass-card px-3 sm:px-4 py-1.5 sm:py-2 rounded-full cursor-default">
              <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-emerald-500 animate-pulse" title="Connected"></div>
              <span className="font-mono text-xs sm:text-sm text-gray-300">{formatAddress(account)}</span>
              <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500 hidden sm:block" />
            </div>
            <button
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-2.5 rounded-full border border-accent-teal text-accent-teal hover:bg-accent-teal hover:text-navy-bg transition-all duration-300 shadow-[0_0_15px_rgba(12,244,198,0.2)] hover:shadow-[0_0_25px_rgba(12,244,198,0.5)]"
          >
            <Wallet className="w-4 h-4 sm:w-5 sm:h-5" />
            <span className="font-semibold text-sm sm:text-base hidden sm:inline">Connect Wallet</span>
            <span className="font-semibold text-sm sm:text-base inline sm:hidden">Connect</span>
          </button>
        )}
      </div>
    </nav>
